...
* **`max_age`** matching the 12-hour cap, plus **`secure: true`** — which is **missing today** — `http_only: true`, `same_site: "Lax"`, `Path=/`, and no `:domain`, so the cookie stays host-scoped rather than shared across `.stump.rocks`.
