 ...
 *
 * Sources: ../docs/adrs (ADR-NNNN-*.md) and ../docs/specs (one directory per
 * capability, each holding spec.md and optionally design.md). Adapted from the
 * same plugin in stump.wtf/harness, which reads docs/openspec/specs instead.
 */
...
function buildMiniDagSection(artifactId, graph) {
  ...
  return [
    ...
    '## Related Artifacts',
    '',
    `Direct relationships declared in this artifact's YAML frontmatter. Run \`/sdd:graph chain ${artifactId}\` for the transitive view.`,
    '',
    '```mermaid',
    mermaid,
    '```',
...
/**
 * Frontmatter values are not guaranteed to be strings: `decision-makers:
 * [joestump]` is a list, and a bare `date: 2026-08-15` can arrive as a Date.
 * Flatten whatever we get into something safe to interpolate into a JSX
 * attribute rather than blowing up mid-build on `.replace is not a function`.
 */
...
function extractMetadataAdr(content) {
  ...
  if (fmMatch) {
    ...
    status = toDisplayString(metadata.status) || null;
    date = toDisplayString(metadata.date) || null;
    dm = toDisplayString(metadata['decision-makers']) || null;
  }
...
function renderHierarchySection({ kind, kindPlural }, graph, baseUrl) {
  ...
  return [
    ...
    '## Hierarchy',
    '',
    `Authored relationships among ${kindPlural} in this project. Cross-kind links (e.g., which ADR a spec implements) appear in each artifact's per-page "Related Artifacts" mini-DAG.`,
    '',
    '```mermaid',
    mermaid,
    '```',
...
function generateDecisionsIndex(adrsSource, docsDest, graph, baseUrl) {
  ...
  for (const file of files) {
    ...
    if (fmMatch) {
      const { metadata } = parseFrontmatter(content);
      status = toDisplayString(metadata.status) || 'unknown';
    }
...
module.exports = function(context, options) {
  ...
  // binnacle keeps specs at docs/specs/, not docs/openspec/specs/.
  const specsDir = opts.specsDir || '../docs/specs';
  ...
  // Read baseUrl and title off the resolved site config rather than by
  // regexing docusaurus.config.ts. The regex approach silently yields an empty
  // baseUrl whenever the config assigns `baseUrl:` from a constant (as this one
  // does), which would emit cross-reference chips as host-root links and break
  // every one of them on a site served under a /<repo>/ path prefix.
  const siteConfig = (context && context.siteConfig) || {};
